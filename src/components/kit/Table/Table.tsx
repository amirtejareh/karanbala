        ...
        <TableHead>
          <TableRow>
            ...
            {headers.map((head, key) => (
              <TableCell key={key} {...head} component={"th"} />
            ))}
            {menuHeader && (
              <TableCell width={150} component={"th"}>
                {menuHeader}
          ...
          <>
            <TableBody>
              {rows?.length > 0 ? (
                rows?.map(
                  (
                    { data, expandNode, expandNodeContent, id, onPressTitle, clickOnIndexId = 0 },
                    key,
                  ) => {
                        ...
                        >
                          ...
                          {expandNode && (
                            <TableCell>
                              ...
                              >
                                ...
                                >
                                  {open[key] ? <BoldArrowDownSvg /> : <BoldArrowUpSvg />}
                                </IconButton>
                              </Box>
                            </TableCell>
                          )}
                          {Object.values(data).map((item, index) =>
                            hasStyle ? (
                              <TableCell
                                onClick={() => {
                                  if (onPressTitle !== undefined && index === clickOnIndexId) {
                                    onPressTitle();
                                  }
                                }}
                                key={index}
