  ...
  return (
    <Box display={"flex"}>
      <Box className={classes.container}>
        <form
          onSubmit={
            value.doUpdate ? handleSubmit(handleUpdateBook) : handleSubmit(handleCreateBook)
          }
        >
          <TextField
            label="عنوان کتاب "
                ...
                getBookReferences?.data.map((element: any) => {
                  return (
                    <MenuItem key={element._id} value={element._id}>
                      {element.title}-{element?.gradeLevels[0]?.title}
                    </MenuItem>
                  );
                })}
            </Select>
          ...
          <Box display={"flex"} position={"relative"} borderRadius={"100%"} mb={3}>
            {selectedFile || preview ? (
              <>
                <Box
                  component={"img"}
                  src={String(preview).split("/")[3] === "undefined" ? BookImage : preview}
                  alt={"test flag"}
                  width={100}
                  height={100}
                />
              ...
              <Box component={"img"} src={BookImage} alt={"User flag"} width={100} height={100} />
            )}
            <IconButton
              sx={{
