  ...
  return (
    <Box display={"flex"}>
      <Box className={classes.container}>
        <form
          onSubmit={
            value?.doUpdate ? handleSubmit(handleUpdateChapter) : handleSubmit(handleCreateChapter)
          }
        >
          <TextField
            label="عنوان فصل "
            ...
            value={value?.data}
            {...register("title", {
              required: "لطفا نام فصل را وارد کنید",
            })}
            inputRef={inputChapterRef}
            onChange={(e) => {
              if (value?.doUpdate) {
      ...
      <Box className={classes.fieldOfStudy}>
        <Typography>لیست فصل‌ها</Typography>
        {!chapters.isLoading ? (
          <TableKit
            secondary
                ...
                data: {
                  ...
                  action: (
                    <>
                      ...
                      <IconButton>
                        <PrompModalKit
                          description={"آیا از حذف فصل مورد نظر مطمئن  هستید؟"}
