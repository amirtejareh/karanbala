            ...
            rows={getObjectiveTestBasedOnGradeLevels?.data?.objectiveTests?.map(
              (item: any, index: any) => {
                return {
                  ...
                  data: {
                    title: `${
                      item?.objectiveTest[0]?.type == "main" ? "اصلی" : "رفع اشکال"
                    } - ${item?.objectiveTest[0]?.number} - ${item?.objectiveTest[0]?.gradeLevel[0]
                      ?.title} - ${item?.bookReferences[0]?.title}`,
                    action: (
                      <>
                        <IconButton
                          onClick={() => {
                            setIsObjectiveTestUpdated(true);
                            setObjectiveTestId(item?.objectiveTest[0]?._id);
                        ...
                        <IconButton>
                          <PrompModalKit
                            ...
                            } - ${item?.objectiveTest[0]?.number} -  ${item?.objectiveTest[0]
                              ?.gradeLevel[0]?.title} - ${item?.bookReferences[0]
                              ?.title}  مطمئن  هستید؟`}
                            onConfirm={() => {
                              handleDeleteObjectiveTestManagement(item._id);
                            }}
                            approved={"بله"}
                            denied={"خیر"}
                          >
