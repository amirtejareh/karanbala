...
import useCreateBookIntro from "../../../../../../hooks/book-intro/useCreateBookIntro";
import useDeleteBookIntro from "../../../../../../hooks/book-intro/useDeleteBookIntro";
import useUpdateBookIntro from "../../../../../../hooks/book-intro/useUpdateBookIntro";
import useGetBookIntroBasedOnBooks from "../../../../../../hooks/book-intro/useGetBookIntroBasedOnBooks";
...
const BookIntro = () => {
  ...
  const createBookIntro = useCreateBookIntro();
  const deleteBookIntro = useDeleteBookIntro();
  const updateBookIntro = useUpdateBookIntro();
  ...
  const handleDeleteBookIntro = (id: string) => {
    deleteBookIntro.mutate(id, {
  ...
  const getBookIntrosBasedOnBooks = useGetBookIntroBasedOnBooks(bookIds);

  useEffect(() => {
    if (getBookIntrosBasedOnBooks.data && !getBookIntrosBasedOnBooks.isLoading) {
      getBookIntrosBasedOnBooks.refetch();
    }
  }, [bookIds]);
  ...
  return (
    <Box className={classes.container}>
        ...
        >
          ...
          <FormControl className={classes.formField} fullWidth>
            <InputLabel id="demo-simple-select-label">
              انتخاب (توضیحات کتاب، کتاب در کنکور، کتاب در امتحان پایانی، بررسی کتاب و نحوه مطالعه)
            </InputLabel>
            <Select
              value={typeIds}
              {...register("type")}
            ...
            >
              <MenuItem value={"bookDescription"}>توضیحات کتاب</MenuItem>
              <MenuItem value={"bookInEntranceExam"}>کتاب در کنکور</MenuItem>
              <MenuItem value={"bookInFinalExam"}> کتاب در امتحان پایانی</MenuItem>
              <MenuItem value={"bookReview"}> بررسی کتاب و نحوه</MenuItem>
            </Select>
          </FormControl>
      ...
      <Box className={classes.fieldOfStudy}>
        <Typography>لیست معرفی کتاب </Typography>
        {!getBookIntrosBasedOnBooks.isLoading ? (
          <TableKit
            secondary
            headers={[{ children: `عنوان` }, { children: `عملیات` }]}
            rows={getBookIntrosBasedOnBooks?.data?.map((item: any, index: any) => {
              return {
                id: item._id,
                data: {
                  title: `${"salam"}`,
                  action: (
                    <>
                      ...
                      <IconButton>
                        <PrompModalKit
                          description={"آیا از حذف ضمیمه مورد نظر مطمئن  هستید؟"}
                          onConfirm={() => handleDeleteBookIntro(item._id)}
                          approved={"بله"}
                          denied={"خیر"}
                        >
                          <DeleteLightSvg width={16} height={16} />
